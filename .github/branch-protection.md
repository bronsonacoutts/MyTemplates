...
| Required status checks                                           | `Validate PR Metadata`, `CI Checks Result`, `Tests Result` | The sequenced ready-for-review workflow chain                           |
...
| Require status checks                 | `Validate PR Metadata`, `CI Checks Result`, `Tests Result` |
...
These are the job names from the ready-for-review workflow chain that should be added as required status checks:

| Workflow File                         | Job Name               |
| ------------------------------------- | ---------------------- |
| `.github/workflows/pr-validation.yml` | `Validate PR Metadata` |
| `.github/workflows/ci.yml`            | `CI Checks Result`     |
| `.github/workflows/tests.yml`         | `Tests Result`         |
...
> Important: the preferred profile only auto-starts these checks when a PR changes from draft to ready for review. Contributors should open PRs as drafts first.
