...
.DESCRIPTION
"Commitship" is the single-phrase trigger for the full governed commit-and-ship workflow.
...
Azure DevOps organisation name. Overrides commitship.config.ps1.

.PARAMETER Project
Azure DevOps project name. Overrides commitship.config.ps1.

.PARAMETER RepositoryName
ADO repository name. Overrides commitship.config.ps1. Defaults to auto-detection from git remote.

.PARAMETER TeamName
ADO team name used for active-sprint iteration lookup. Overrides commitship.config.ps1.

.PARAMETER TargetBranch
Branch that PRs will target. Defaults to 'main'.

.PARAMETER AreaPath
Default area path for new work items. Overrides commitship.config.ps1.

.PARAMETER IterationPath
Default iteration path for new work items. Overrides commitship.config.ps1.
...
.EXAMPLE
# Triggered by the 'commitship' phrase — interactive, confirms everything
...
./scripts/Invoke-Commitship.ps1 -Organization 'myorg' -Project 'MyProject'
#>
...
# ---------------------------------------------------------------------------
# Load commitship.config.ps1 (next to script, or at repo root)
# ---------------------------------------------------------------------------
function Import-CommitshipConfig {
    $candidates = @(
        (Join-Path $root 'commitship.config.ps1'),
        (Join-Path (git rev-parse --show-toplevel 2>$null).Trim() 'commitship.config.ps1')
    )
...
function Get-RepoId {
    ...
    if ([string]::IsNullOrWhiteSpace($script:RepositoryName)) {
        throw "Unable to determine repository name. Set RepositoryName in commitship.config.ps1 or pass -RepositoryName."
    }
...
function Get-SuggestedDescription {
    ...
    return "Changes in topic area: $Topic.`n`nAffected files:`n$fileList`n`nGenerated by Invoke-Commitship.ps1 on $(Get-Date -Format 'yyyy-MM-dd HH:mm')."
}
...
if ([string]::IsNullOrWhiteSpace($Organization)) { throw "Organization is required. Set it in commitship.config.ps1 or pass -Organization." }
if ([string]::IsNullOrWhiteSpace($Project))      { throw "Project is required. Set it in commitship.config.ps1 or pass -Project." }
if ([string]::IsNullOrWhiteSpace($AreaPath))     { throw "AreaPath is required. Set it in commitship.config.ps1 or pass -AreaPath." }

Write-Step 'COMMITSHIP — Governed Commit & Ship Workflow'
...
foreach ($topic in $groupKeys) {
    ...
    if ($AutoWorkItems) {
        ...
        $wiTags     = 'auto-created;commitship'
        $wiAcText   = ''
        $doCreate   = $true
        $existingId = 0
    } else {
        ...
        if (-not [string]::IsNullOrWhiteSpace($existingIdStr) -and $existingIdStr -match '^\d+$') {
            ...
            $wiTags   = Read-UserInput "  Tags (semicolon separated)" 'auto-created;commitship'
            Write-Host ''
...
---
*Created by Invoke-Commitship.ps1*
"@
...
# 8. Summary
Write-Step 'Commitship complete'
